...
function IndustryAgentCards() {
  // By default, all cards are flipped (showing image). On hover, show text side.
  const [flippedCards, setFlippedCards] = useState<number[]>(Array.from({length: 6}, (_, i) => i));
  ...
  const handleMouseEnter = (index: number) => {
    setFlippedCards(prev => prev.filter(i => i !== index)); // Remove index to flip to text
  };
  const handleMouseLeave = (index: number) => {
    setFlippedCards(prev => [...prev, index]); // Add index to flip back to image
  };
